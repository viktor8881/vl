        ...
        if ($coll->count()) {
            foreach ($coll as $invest) {
                $html .= '<tr>';
                    ...
                    $html .= '<td><a href="/investments/metal/edit/id/'.$invest->getId().'">'.$this->view->iconEdit('edit')._('ред.').'</a> '
                            . '<a href="/investments/metal/delete/id/'.$invest->getId().'">'.$this->view->iconDelete('del')._('уд.').'</a></td>';
                $html .= '</tr>';
            }
        }else{
